...
- 新增测试覆盖工作日 / 跨午夜 / 周末边界、静默期间保留暴露、15 秒恢复缓冲、预约窗口 / 午夜截止 / 重启去重 / 时钟回拨 / 旧数据兼容。
- Windows 检查新增真实控制器的会议等待、恢复缓冲、7 分钟邀请及非强制开始、预约持久化、保存失败不误报；截图新增安排页和紧凑布局。
- [Windows CI](https://github.com/yearliny/workguard/actions/runs/34134914375) 已通过完整编译、真实 WPF 交互与精简包启动检查，保存 32 个界面状态。已查看安排页、预约邀请、设置与紧凑布局；原生截图见 [我的安排](images/daily-plan-v0.7.png)。
- 根据截图压缩安排页间距，保证默认窗口内显示主要卡片；小窗口缩放后自动将编辑中的时间输入框完整滚入视野。
- [PR #6](https://github.com/yearliny/workguard/pull/6) 跟踪最终提交与构建。继续保留 Win10 / Win11、混合 DPI、多屏、辅助技术、长时间使用与签名的实机验收边界。
...
- [Windows CI](https://github.com/yearliny/workguard/actions/runs/34112622844) 已通过真实 WPF 交互与精简包检查，保存 27 个界面状态。已查看入门、紧凑错误页、动作路线和未完整结束截图，见 [活动路线](images/activity-route-v0.6.png)。
- [PR #5](https://github.com/yearliny/workguard/pull/5) 跟踪最终构建；保留现有 Win10 / Win11 实机验收边界。
...
- [PR #3](https://github.com/yearliny/workguard/pull/3) 的 [Windows CI](https://github.com/yearliny/workguard/actions/runs/34099234223) 已通过实际窗口与发布包检查，涵盖：显示即开始、隐藏及拒绝普通跳过 / 暂停、关闭确认、完成自动释放、紧急退出不计数、系统暂停后继续、替代休息统计集成、辅助屏窗口释放。
