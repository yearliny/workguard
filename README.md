...
**0.9.0-preview.1 每日身体维护（已发布）** · C# / .NET 10 / WPF · 无账户、无服务器、无广告、无应用遥测
...
## 0.9.0-preview.1：每日身体维护试用

开发分支已接入每日 5–10 分钟跟练目标、短维护与集中维护、按部位覆盖的 13 个基础动作、原生矢量示意、可选系统中文语音及用户确认记录。自动融入身体休息需单独开启；颈部动作仍默认关闭，不适时可以立即退出并暂停对应动作。

78/78 项核心测试、Windows 编译、原生交互和精简包启动检查已通过；已检查默认及紧凑窗口截图。[v0.9.0-preview.1 下载](https://github.com/yearliny/workguard/releases/tag/v0.9.0-preview.1)。具体范围、数据升级及仍需实机体验的项目见 [每日维护实现说明](docs/DAILY-MAINTENANCE.md)。

下一版 0.9.0-preview.2 正在完善跟练：开始前逐个预览动作、切换左右侧与播放示意；跟练中示意与当前侧同步。强制退出确认暂停计时，取消退出后保留原有暂停与系统中断状态。验证结果由对应 PR 跟踪。
...
- 78 项核心行为测试；Windows 原生窗口、交互及发布包检查；CI 保存界面截图，主分支通过检查后发布精简包。
...
- 每日维护提供原生动作示意与可选系统中文语音，普通休息仍使用文字指引和下一步预告。暂无自动更新、安装器、代码签名。
- 全屏检测为窗口尺寸启发式；无边框窗口、特殊窗口管理器等场景需要实机验证。
- 普通休息统计为流程完成次数，每日维护统计为用户确认的跟练时长；都不能证明动作质量、离开座位或健康改善。自然离席不增加完成次数。
