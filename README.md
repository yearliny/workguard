...
**0.5.0 视觉与动效更新（预发布）** · C# / .NET 10 / WPF · 无账户、无服务器、无广告、无应用遥测
...
- 43 项核心行为测试；Windows 原生窗口、交互及发布包检查；CI 保存界面截图并自动发布精简包。
...
安装 .NET 10 Desktop Runtime x64，完整解压 `WorkGuard-0.5.0-win-x64-lite.zip` 后运行 `WorkGuard.exe`。请保留目录中的所有文件。当前没有代码签名、安装器或自动更新。更新前先从托盘退出旧版本，再完整解压新版本；设置和统计会继续使用原数据目录。
