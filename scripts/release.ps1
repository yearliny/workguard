...
- 三步快速开始：选择节奏、提醒力度，确认保存；支持返回修改与保存失败重试。
- 现有用户可从概览进入快速配置，保留未展示的高级偏好与自定义节奏。
- 身体活动页新增当前与接下来三个环节的路线预告，安静替代自动隐藏动作。
- 跳过后的结束页显示实际进度；完整休息的轻量反馈在概览停留两分钟。
- 沿用离线插画与短时原生动画；保留强制休息、紧急退出与减少动态效果。
- 45 项核心测试，以及 Windows 引导、交互、动画和精简包检查。
